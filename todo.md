...
- [x] สร้างหน้า Landing ที่แสดงโลก Arcane Frontier, ปุ่มเริ่มเล่น และปุ่มดาวน์โหลด/เพิ่มไปยังหน้าจอหลัก โดยจัดวางให้เหมาะกับจอมือถือแนวนอน
- [x] ใช้ Gemini API กำหนด art direction, color system, UI hierarchy และรายละเอียด layout สำหรับโลกแฟนตาซีไซไฟที่สง่างาม
- [x] เพิ่ม Babylon.js พร้อมโครงสร้างเกมแยกจาก React และ canvas ที่รองรับ lifecycle ของ React อย่างปลอดภัย
- [x] สร้างฉากโลก open-world ขนาดต้นแบบที่มีพื้นที่รกร้าง, ซากเทคโนโลยีเอเลี่ยน, แหล่งทรัพยากร และศัตรูซอมบี้
- [x] เพิ่มผู้เล่นที่เดินสำรวจ, วิ่ง, โจมตี และมีค่าสุขภาพ/พลังชีวิตที่เปลี่ยนแปลงได้
- [x] เพิ่ม AI ศัตรูซอมบี้/มอนสเตอร์ที่ตรวจจับผู้เล่น โจมตี และทิ้งทรัพยากรเมื่อถูกกำจัด
- [x] เพิ่มการเก็บทรัพยากรและคลังไอเทม พร้อมช่องลัดไอเทมใน HUD
- [x] เพิ่มระบบสร้างสิ่งปลูกสร้างขนาดต้นแบบ เช่น กองไฟ ผนัง และแปลงปลูกพืช
- [x] เพิ่มวงจรปลูกพืช รดน้ำ/รอเติบโต และเก็บเกี่ยวเป็นทรัพยากร
- [x] เพิ่มคอนโทรลทัชสำหรับจอแนวนอน: joystick เดิน ปุ่มโจมตี ปุ่มโต้ตอบ ปุ่มสร้าง และแถบไอเทม
- [x] เพิ่มโหมดเดโมที่กำหนดสถานะเริ่มต้นตายตัวเพื่อให้ตรวจสอบภาพหน้าจอได้
- [x] สร้างระบบเข้าเกมด้วย Player ID เพียงครั้งเดียวต่ออุปกรณ์ โดยไม่ใช้ Manus OAuth กับเส้นทางเกม
- [x] บันทึกสถานะเกมลง localStorage เพื่อให้เล่นต่อแบบออฟไลน์ได้
- [x] สร้างตารางฐานข้อมูลและ tRPC สำหรับโปรไฟล์ สถานะการเซฟ คลังไอเทม และบันทึก provenance ของไอเทม
- [x] ซิงก์สถานะเกมและคลังไอเทมเมื่อออนไลน์ โดยมีคิวการเปลี่ยนแปลงสำหรับส่งภายหลัง
- [x] ตรวจสอบ provenance ของไอเทมสำคัญเมื่อซิงก์ โดยยอมรับเฉพาะแหล่งที่มาจาก drop, craft, harvest หรือ reward ที่ตรวจสอบย้อนหลังได้
- [x] เพิ่ม manifest และ service worker เพื่อให้ PWA-ready และรองรับการเปิดต้นแบบที่แคชไว้แบบออฟไลน์
- [x] เขียน Vitest ครอบคลุมกฎ provenance, การรวมสถานะซิงก์ และกติกาการต่อสู้หลัก
- [x] ตรวจสอบภาพบนขนาดมือถือแนวนอนและเดสก์ท็อป แก้ไขข้อผิดพลาด TypeScript และบันทึก checkpoint ก่อนส่งมอบ
- [x] สร้าง flow เข้าเกมด้วย Player ID แบบไม่ต้องสมัคร: ตรวจหาโปรไฟล์เดิมหรือสร้างโปรไฟล์และคลังไอเทมเริ่มต้นทันที
- [x] สร้างหน้า Lobby ที่แสดงตัวละคร อุปกรณ์ที่สวมใส่ ค่าพลัง ปุ่มร้านค้า การแต่งตัว กิจกรรม และปุ่มเลือกแผนที่
- [x] สร้างหน้าเลือกแผนที่ที่กำหนด biome ระดับความเสี่ยง และระยะทาง/ขนาดพื้นที่ของแต่ละฉากในต้นแบบ
- [x] ปรับกล้องและการเคลื่อนที่เป็น top-down/isometric แบบ MOBA สำหรับควบคุมด้วย joystick บนจอแนวนอน
- [x] สร้างร้านค้าและหน้าปรับแต่งตัวละครสำหรับสีผม ชุด และช่องอุปกรณ์ โดยใช้ข้อมูลไอเทมจากคลัง
- [x] ออกแบบระบบอาวุธไร้คลาสที่รองรับปืน มีด ดาบ คฑา หนังสือ และอาวุธระดับตำนาน พร้อม status effect/debuff ตามชนิดอุปกรณ์
- [x] ออกแบบโครงสร้างรองรับหลายฉากและการส่งผ่านสถานะผู้เล่นระหว่าง Lobby แผนที่ และโลกเอาชีวิตรอด
...
- [x] สร้างแผนที่ biome ตัวอย่างที่เล่นได้จริง พร้อมทรัพยากร ศัตรู และบรรยากาศที่สอดคล้องกับธีมเดียวกันทั้งฉาก
- [x] สร้างระบบเลือกและโหลดแผนที่ตามต้องการ พร้อมหน้าจอเปลี่ยนฉากที่มี progress เป็นเปอร์เซ็นต์และเอฟเฟกต์ตาม biome
...
- [x] ใช้ Gemini API กำหนด design bible ของตัวละครอนิเมชัน เอฟเฟกต์สกิล อาวุธ ไอเทม และ UI โดยหลีกเลี่ยงภาพ 3D รูปทรงเรขาคณิตแบบเรียบง่าย
- [x] สร้างตัวละครผู้เล่นสไตล์อนิเมชันพร้อม silhouette, palette และ state ภาพสำหรับ idle เดิน โจมตี และได้รับความเสียหาย
- [x] เพิ่มเอฟเฟกต์ภาพของการเดิน การโจมตี พลังอาคม อาวุธ และการเก็บไอเทมที่ปรับระดับความละเอียดได้
...
- [x] เพิ่ม lighting, color grading, shadow, ambient sound และ motion effects ที่เปลี่ยนตามเวลาและ biome โดยเชื่อมกับการตั้งค่าคุณภาพ
- [x] เพิ่ม weekly event panel ใน Lobby สำหรับแสดงกิจกรรม ระยะเวลาคงเหลือ เป้าหมาย และรางวัลของสัปดาห์
- [x] แสดงเวอร์ชันเกม 100.1.1.1 แบบไม่เด่นในหน้า Lobby และเก็บ versioning policy: ใหญ่.แผนที่หรือระบบ.แพตช์.กิจกรรม
...
- [x] commit และ push การเปลี่ยนแปลงไปยัง A_Survival หลังแผนที่หนึ่งเสร็จตามเกณฑ์ พร้อมข้อความ commit ที่ระบุชื่อแผนที่
...
- [x] ปรับ Lobby เป็น cinematic character hub ที่มีเมนูแนวตั้งด้านซ้าย แถบทรัพยากร/แจ้งเตือนด้านบน ตัวละครเด่นกลางฉาก ปุ่มโหมดด้านล่าง และ event/ทางลัดด้านข้างในสไตล์ Arcane Frontier
- [x] ปรับกล้องแผนที่เป็น top-down 3/4 แบบ MOBA ที่ตามผู้เล่น มองพื้นที่ต่อสู้และรักษา safe zone สำหรับ touch HUD จอแนวนอน
- [x] ทำให้หน้าเข้าเกมรับเพียงชื่อหรือ Player ID โดยไม่มี Gmail รหัสผ่าน หรือการยืนยันบัญชี และให้สร้าง/เปิดเซฟในเครื่องได้ทันทีเมื่อออฟไลน์
- [x] ซิงก์ profile และคลังไอเทมของชื่อที่กรอกแบบเบื้องหลังเมื่อออนไลน์ โดยไม่ขัดขวางการเล่นหรือบังคับล็อกอิน
...
- [x] ยกระดับ MAP_010 Void-Infused Rift ด้วย Pollinations asset pack, deterministic void-rift safe-zone loop, elite/boss telegraph และ reward provenance